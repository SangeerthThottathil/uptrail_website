...
export async function generateMetadata({
  ...
  if (!programme || programme.track !== 'certification') return { title: 'Certification — Uptrail' }
  return {
...
export default async function CertificationDetailPage({
  ...
  if (!programme || programme.track !== 'certification') notFound()

  const included = [
    'Live weekly sessions with expert instructors',
    'One-to-one mentorship and weekly feedback',
    'Hands-on projects with real-world datasets',
    'Full exam preparation and practice tests',
    'A global peer community and alumni network',
    'A globally recognised industry certification',
  ]

  return (
    <main>
      <PageHeader
        label={`${programme.category} certification`}
        title={programme.title}
        description={programme.blurb}
        image={programme.image}
        imageAlt={`Learners on the ${programme.title} certification`}
      />

      <ProgrammeOverview programme={programme} kind="certification" />
      <CareerOutlook programme={programme} />
      <WhyChoose programme={programme} kind="certification" />
      ...
      <WhoIsItFor kind="certification" />
      <HowItWorks kind="certification" />
      <ProviderComparison />
      ...
      <PaymentOptions kind="certification" options={programme.paymentOptions} programmeSlug={programme.slug} />
